...
# ============================================
# 📤 تعيين القناة مباشرة (هنا تحط معرف قناتك)
# ============================================

# 🔴 غيّر هذا الرقم إلى معرف قناتك الحقيقي
# خذه من بوت @getmyid_bot
CHANNEL_ID = -1003886889715  # <--- ضع معرف قناتك هنا
...
#تحميل #{media_type} #{platform if platform != 'search' else 'بحث'}
        ...
        # إرسال الملف مع النص إلى القناة
        if file_path and os.path.exists(file_path):
            ...
            # إذا لم يوجد ملف (مثل الصور من البحث)
            msg = bot.send_message(CHANNEL_ID, post_text, parse_mode="Markdown")
        
        # تخزين المنشور في قاعدة البيانات
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        # إشعار المستخدم بأن طلبه تم نشره
        try:
            bot.send_message(
                user_id,
...
# ============================================
# 📊 دوال إدارة قاعدة البيانات (الكود الأصلي)
# ============================================
...
# ============================================
# 🎨 واجهة الأزرار المتطورة (القائمة الرئيسية الجديدة)
# ============================================
...
def show_main_menu(chat_id, user_id=None):
    ...
    markup.add(
        InlineKeyboardButton("🔍 بحث سريع", callback_data="menu_search", style="primary"),
        InlineKeyboardButton("📥 تحميل من رابط", callback_data="menu_download", style="success")
    )
    
    markup.add(
        InlineKeyboardButton("🏆 لوحة المتصدرين", callback_data="menu_leaderboard", style="primary"),
        InlineKeyboardButton("🎁 المكافآت اليومية", callback_data="menu_daily", style="success")
    )
    
    markup.add(
        InlineKeyboardButton("📊 إحصائياتي", callback_data="menu_my_stats", style="primary"),
        InlineKeyboardButton("📚 المكتبة", callback_data="menu_library", style="primary")
    )
    
    markup.add(
        InlineKeyboardButton("🎯 توصيات", callback_data="menu_recommend", style="primary"),
        InlineKeyboardButton("🔄 تحميل تلقائي", callback_data="menu_auto_download", style="success")
    )
    
    markup.add(
        InlineKeyboardButton("💬 تواصل مع المطور", url=f"https://t.me/{OWNER_USERNAME}", style="primary"),
        InlineKeyboardButton("❓ مساعدة", callback_data="menu_help", style="primary")
    )
    
    if user_id == OWNER_ID:
        markup.add(
            InlineKeyboardButton("⚙️ لوحة التحكم", callback_data="menu_admin_panel", style="danger"),
            InlineKeyboardButton("📢 إدارة القناة", callback_data="menu_channel_settings", style="primary")
        )
    
    welcome_text = """
🏰 **القائمة الرئيسية - البوت الإمبراطوري**
━━━━━━━━━━━━━━━━━━━━━━━

📌 **اختر الخدمة التي تريدها من الأزرار أدناه:**

🔍 **بحث سريع** - ابحث عن صور، فيديوهات، صوتيات
📥 **تحميل من رابط** - حمّل من 7 منصات مختلفة
🏆 **لوحة المتصدرين** - شاهد أفضل المستخدمين
🎁 **المكافآت اليومية** - احصل على مكافأتك اليومية
📊 **إحصائياتي** - تتبع تقدمك في البوت
📚 **المكتبة** - استعرض أحدث المحتويات
🎯 **توصيات** - اقتراحات مخصصة لك
🔄 **تحميل تلقائي** - فعّل التحميل التلقائي

💡 **نصيحة:** كل تحميل يمنحك نقاط تصل إلى متجر البوت!
    """
                ...
                try:
                    ...
                    for pattern in patterns:
                        ...
                        if matches:
                            ...
                            # 🔴 نشر في القناة
                            post_to_channel(chat_id, "image", "search", query, 0)
                    ...
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        if info and 'entries' in info and len(info['entries']) > 0:
                            ...
                            if os.path.exists(filename):
                                ...
                                # 🔴 نشر في القناة
                                post_to_channel(chat_id, media_type, "search", query, file_size, downloaded_file_path)
...
def download_media_processor(url, chat_id, reply_to_id, media_type, quality=None, platform="unknown"):
    """معالجة تحميل الوسائط مع إضافة النشر في القناة"""
...
📈 النقاط المتبقية للمستوى التالي: {100 - (downloads % 100)}
            ...
            bot.send_message(chat_id, "🔄 **تم تفعيل التحميل التلقائي!**\nسيتم تحميل أي رابط ترسله تلقائياً.")
...
def show_channel_settings(chat_id):
    ...
    channel_status = "✅ تم التعيين" if CHANNEL_ID else "❌ لم يتم التعيين"
    text = f"""
⚙️ **إعدادات القناة**
━━━━━━━━━━━━━━━━━━━
📌 حالة القناة: {channel_status}
🆔 معرف القناة: {CHANNEL_ID or 'غير محدد'}

📝 **ملاحظة:** 
- ستنشر جميع طلبات المستخدمين في القناة
...
print("📤 نظام النشر في القناة مع إخفاء الهوية (مفعل تلقائياً)")
print("🏆 نظام النقاط والمكافآت")
print("📚 نظام المكتبة والأرشيف")
print("🎯 نظام التوصيات الذكي")
print("🎨 واجهة أزرار متطورة")
print("🌐 دعم منصات إضافية")
print("📢 نظام الإشعارات الذكي")
print("=" * 60)
print("✅ البوت جاهز للاستخدام مع جميع الإضافات!")
print("💡 استخدم /menu لعرض القائمة الرئيسية الجديدة")
print("💡 استخدم /points لعرض نقاطك")
print("💡 استخدم /library لعرض المكتبة")
print("=" * 60)
